<?php
 ...
 class venta extends sistema {
    ...
    function readAll (){
        $this -> conexion();
        $result = [];
        $consulta ='select v.*, u.nombre_completo, p.nombre_producto from venta v inner join usuario u on v.usuario_id = u.id inner join producto p on v.producto_id = p.id;';
        $sql = $this->con->prepare ($consulta); 
        $sql -> execute();
