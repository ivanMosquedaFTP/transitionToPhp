<?php
    ...
    class sistema {
        ...
        function getRole($emaill) {
            $this -> conexion();
            $data = [];
            if (filter_var($emaill, FILTER_VALIDATE_emailL)) {
                $sql = "select r.rol from usuario u inner join usuario_rol ur on u.id = ur.id_usuario
                    inner JOIN rol r on r.id_rol = ur.id_rol
                    where u.emaill = :emaill ";

                $roles = $this->con->prepare($sql);
                $roles->bindParam(":emaill",$emaill,PDO::PARAM_STR);
                $roles->execute();
        ...
        function getPrivilegios($emaill) {
            $this -> conexion();
            $data = [];
            if (filter_var($emaill, FILTER_VALIDATE_emailL)) {
                ...
                $privilegio->bindParam(":emaill",$emaill,PDO::PARAM_STR);
                $privilegio->execute();
